...
- Honda e:Ny1 equivalents (`ev_ohme_sync_honda_state_of_charge`, `ev_ohme_auto_approve_honda_charge`, added 2026-09-01) mirror the Renault pair for the second car on the shared charger, with two deliberate differences. (1) **Freshness gating**: Honda cloud data can be many hours stale, so both Honda automations press `button.e_ny1_refresh_from_car` and require `sensor.e_ny1_last_updated` to be within 5 minutes before trusting any Honda reading, aborting on a 2-minute timeout rather than acting on a stale value. The press is wrapped in `continue_on_error: true` — it *raises* ("Unable to refresh data from vehicle") when the car is asleep or out of signal, and on 2026-09-03 that unhandled error killed both Honda runs at `action/0` before the freshness wait could abort them cleanly. (2) A 15-minute `time_pattern` tick forces a Honda refresh while Ohme is `charging` **or** `pending_approval`, because the Honda does not stream updates the way the Renault does. It deliberately does NOT require the Honda to already be selected in Ohme — the Honda is only ever selected *by* that automation, so requiring it made the tick unable to bootstrap. Identification uses `device_tracker.e_ny1_location` (GPS), not `sensor.e_ny1_home_away` — those two disagree.
- **Shared car-identity contract for all four EV automations** (reworked 2026-09-03; supersedes the 2026-09-02 "settle then decide" gate, which in turn replaced a "Renault wins ties" rule — both proved wrong in production). Identity is never read at trigger time and never sampled instantaneously. Each run: presses the Honda refresh (guarded), **waits** for a shared claim gate — `renault_claims != honda_claims`, 6-minute timeout, `continue_on_timeout: false` — then holds `00:01:00` and **re-checks** the same template as a hard condition (`r and not h`, or `h and not r`). Three rules make this work and must not be relaxed: (a) **stale data is not a claim** — the Honda only claims the cable if `sensor.e_ny1_last_updated` is under 30 minutes old, because on 2026-09-03 its `plug_status` froze at `plugged_in` for 12 hours and permanently deadlocked the gate; (b) **a car that has not caught up is waited for, never rejected** — the Renault's plug sensor lagged the charger by 4m05s and its GPS by a further 4s, and the old hard `condition: state ... home` failed by those 4 seconds and left a real session unapproved for 3 hours; (c) **the settle delay is the guard against a briefly-wrong sensor** — on 2026-09-02 the Renault's plug sensor read `on` for 46s after it was unplugged and approved a Honda session. Both SoC-match waits also require `select.ohme_home_pro_vehicle` to name the expected car, since a bare number match can be satisfied by the outgoing car's figure. Because every trigger is edge-based, both approve automations carry a `/5` `time_pattern` retry tick (self-limiting via the `pending_approval` condition) — without it a single failed run strands the session until someone taps the app.
