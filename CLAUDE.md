...
- Honda e:Ny1 equivalents (`ev_ohme_sync_honda_state_of_charge`, `ev_ohme_auto_approve_honda_charge`, added 2026-09-01) mirror the Renault pair for the second car on the shared charger, with three deliberate differences. (1) **Freshness gating**: Honda cloud data can be many hours stale, so both automations press `button.e_ny1_refresh_from_car` and require `sensor.e_ny1_last_updated` to be within 5 minutes before trusting any Honda reading, aborting on a 2-minute timeout rather than acting on a stale value. (2) **Settle then decide** (reworked 2026-09-02, replacing an earlier "Renault wins ties" rule that proved wrong): car identity is NOT read at trigger time. All four EV automations check identity inside the action sequence, behind a shared gate that waits until exactly one car claims the cable — `(renault_plugged and renault_home) != (honda_plugged and honda_home)` — 3-minute timeout, no decision means no action. This exists because on 2026-09-02 the Renault's plug sensor stayed `on` for 46s after it was unplugged and the Renault automation approved a Honda session. Both SoC-match waits also require `select.ohme_home_pro_vehicle` to name the expected car, since a bare number match can be satisfied by the outgoing car's figure. (3) A 15-minute `time_pattern` tick forces a Honda refresh mid-charge, gated on the Honda already being the selected vehicle and Ohme being `charging`, because the Honda does not stream updates the way the Renault does. Identification uses `device_tracker.e_ny1_location` (GPS), not `sensor.e_ny1_home_away` — those two disagree.
