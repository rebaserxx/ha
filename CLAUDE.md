...
- Tado gas meter daily sync (`tado_gas_meter_reading_daily_from_octopus`) derives a cumulative meter register from Octopus daily-usage sensors and submits it to Tado as an integer; state is carried in `input_number.tado_gas_meter_register_m3` / `input_datetime.tado_gas_meter_last_submission_date`. Manual correction via `script.tado_gas_set_manual_baseline`.
